...
parser.add_argument('--results_path', type=str, default='results/isalis/model_3', help='Path to save the results')
...
def main_3():
    ...
    train_ddi_df, val_ddi_df, test_ddi_df = split_data(df_DDI) # train: 70%, val: 10%, test: 20%
    ...
    metrics_ddi = evaluate(model, test_ddi_loader, args.results_path, "model_3_ddi", device)
    metrics_ham = evaluate(model, test_ham_loader, args.results_path, "model_3_ham", device)
    print("Best model:")

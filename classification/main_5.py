...
parser.add_argument('--results_path', type=str, default='results/isalis/model_5', help='Path to save the results')
...
def main():
    ...
    df_AUG = load_generated_metadata('../data/HAM10000/HAM10000_metadata.csv', '../data/darkHAM_sigproc')
    ...
    df_AUG['image_id'] = df_AUG['image_path'].apply(lambda x: x.split('_fake_Y')[0]) + '.png'
    ...
    metrics_ddi = evaluate(model, test_ddi_loader, args.results_path, "model_5_ddi", device)
    metrics_ham = evaluate(model, test_ham_loader, args.results_path, "model_5_ham", device)
    print("Best model:")

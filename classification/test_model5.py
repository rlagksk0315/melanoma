...
best_loss_ckpt = os.path.join('/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', 'model_best_loss.pth')
state = torch.load(best_loss_ckpt, map_location=device)
model.load_state_dict(state['model_state_dict'])
model.to(device)
print("Best-loss DDI: ", evaluate(model, test_ddi_loader, '/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', "loss_model_5_ddi", device))
print("Best-loss HAM: ", evaluate(model, test_ham_loader, '/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', "loss_model_5_ddi", device))

# evaluate checkpoint with best validation accuracy
best_acc_ckpt = os.path.join('/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', 'model_best_acc.pth')
state = torch.load(best_acc_ckpt, map_location=device)
model.load_state_dict(state['model_state_dict'])
model.to(device)
print("Best-acc DDI: ", evaluate(model, test_ddi_loader, '/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', "acc_model_5_ddi", device))
print("Best-acc HAM: ", evaluate(model, test_ham_loader, '/projects/pancreas-cancer-hpc/hana-eus/melanoma/results/hana/main_5_classification', "acc_model_5_ham", device))
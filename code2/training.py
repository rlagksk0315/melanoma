...
# Dark skin data loaders - concatenate ddi and dark_scin 
darkskin_loader_train = torch.utils.data.ConcatDataset([dark_scin_loader_train.dataset, ddi_loader_train.dataset])
darkskin_loader_val = torch.utils.data.ConcatDataset([dark_scin_loader_val.dataset, ddi_loader_val.dataset])
darkskin_loader_test = torch.utils.data.ConcatDataset([dark_scin_loader_test.dataset, ddi_loader_test.dataset])

# Light skin data loaders - concatenate ham and light_scin 
lightskin_loader_train = torch.utils.data.ConcatDataset([light_scin_loader_train.dataset, ham_loader_train.dataset])
lightskin_loader_val = torch.utils.data.ConcatDataset([light_scin_loader_val.dataset, ham_loader_val.dataset])
lightskin_loader_test = torch.utils.data.ConcatDataset([light_scin_loader_test.dataset, ham_loader_test.dataset])
...
def train_cyclegan(lightskin_loader_train, darkskin_loader_train, generate_XtoY, generate_YtoX, Discriminator_X, Discriminator_Y, device):
    ...
    loop = tqdm(zip(lightskin_loader_train, darkskin_loader_train), total=min(len(lightskin_loader_train), len(darkskin_loader_train)))
    for ham_batch, darkskin_batch in loop:
        ...
        loss_generator = (
            loss_G_XtoY + loss_G_YtoX + 10 * (loss_cycle_X + loss_cycle_Y) + 10 * (loss_identity_X + loss_identity_Y)
        )
    ...
    with open("cyclegan_training_log.txt", "w") as f:
        ...
        for epoch in range(epochs):
            ...
            train_generator_loss, train_discriminator_X_loss, train_discriminator_Y_loss = train_cyclegan(
                lightskin_loader_train = lightskin_loader_train,
